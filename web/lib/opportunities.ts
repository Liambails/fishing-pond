...
function latestPrice(l:any){for(const o of [...(l.observations||[])].sort((a:any,b:any)=>Date.parse(b.captured_at)-Date.parse(a.captured_at))){const p=o?.sold_price_nzd??o?.buy_now_nzd??o?.asking_price_nzd??o?.starting_price_nzd??o?.current_bid_nzd??null;if(p!=null&&Number.isFinite(Number(p)))return Number(p)}return null}
