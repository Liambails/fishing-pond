...
export async function detectNewIdRelist(db:any,listing:any,obs:any){
 ...
 const sameSeller=Boolean(norm(listing.seller)&&norm(best.old.seller)&&norm(listing.seller)===norm(best.old.seller));
 const oldEvidence={...(best.old.final_evidence||{}),relist:{detected:true,successor_listing_id:listing.listing_id,same_seller:sameSeller,detection_method:'semantic_new_id',detected_at:now,confidence:best.match.score}};
 await db.from('listings').update({active:false,lifecycle_state:'relisted',next_observation_at:null,relist_successor_uuid:listing.id,last_relist_checked_at:now,closure_reason:`relisted to #${listing.listing_id}`,cadence_reason:`relisted as marketplace listing ${listing.listing_id}`,final_evidence:oldEvidence}).eq('id',best.old.id);
