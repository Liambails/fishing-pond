...
export async function POST(req:Request){
 try{
  ...
  const {data:obs}=ids.length?await db.from('observations').select('listing_uuid,captured_at,views,watchers,bids,buy_now_nzd,asking_price_nzd,current_bid_nzd,sold_price_nzd,close_date,close_remaining,question_count,purchase_intent_questions,compatibility_questions,condition_questions,buy_now_available,offer_available,stock_quantity,listing_status,sold_detected,qa_identity_codes').in('listing_uuid',ids).order('captured_at',{ascending:false}).limit(1000):{data:[] as any[]};
