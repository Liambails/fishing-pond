...
import {buildIdf,genericListingSimilarity,listingDocument} from '../../../../lib/genericSimilarity';
...
export async function GET(req:Request){
 try{
  ...
  const scored=(listings||[]).filter((l:any)=>l.id!==source.id).map((l:any)=>({listing:l,match:genericListingSimilarity(source,l,idf,sourceExtra,latestObservationExtra(latest.get(l.id)))})).filter((x:any)=>x.match.score>=.34).sort((a:any,b:any)=>b.match.score-a.match.score).slice(0,30);
