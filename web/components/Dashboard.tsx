...
export default function Dashboard({products,listings,interventions:initialErrors,systemEvents:initialSystemEvents,aiEnabled}:Props){
 ...
 return <main className="appShell">
  <header className="appHeader"><div><div className="brandSmall">MOTERA RESEARCH LAB</div><div className="brandTitle cobaltWordmark">COBALT <span>V3.9.2</span></div></div><div className="headerMeta headerClock">{clock?formatNZDateTime(clock,true):'—'}</div></header>
  ...
  {selected&&<div className="modalBack" onMouseDown={e=>{if(e.target===e.currentTarget)setSelected(null)}}><div className="modal crmModal"><div className="modalTitleBar"><div><span className="windowKicker">PRODUCT CRM</span><h2>{selected.name}</h2></div><button className="windowClose" onClick={()=>setSelected(null)}>×</button></div><div className="modalBody">
   ...
   <details className="classicDetails"><summary>All competitor listings ({selected.listings.length})</summary><div>{selected.listings.map((l:any)=><p key={l.id}><a href={l.url} target="_blank" rel="noreferrer">#{l.listing_id} · {l.title}</a> — {l.seller||'unknown seller'}{l.comparableMatch?.match_score!=null&&<span className="secondary"> · {Math.round(Number(l.comparableMatch.match_score)*100)}% comparable · {l.comparableMatch.match_method||'matched'}</span>}</p>)}</div></details>
   {selected.reviewMatches?.length>0 ? (
    <details className="classicDetails"><summary>Possible competitors to review ({selected.reviewMatches.length})</summary><div>{selected.reviewMatches.map((m:any)=><p key={m.listing_uuid}><a href={m.listing.url} target="_blank" rel="noreferrer">#{m.listing.listing_id} · {m.listing.title}</a> — {Math.round(Number(m.score)*100)}% match <button className="smallBtn" disabled={busy} onClick={()=>decideComparable(selected.id,m.listing_uuid,'accept')}>Accept</button> <button className="smallBtn" disabled={busy} onClick={()=>decideComparable(selected.id,m.listing_uuid,'reject')}>Reject</button></p>)}</div></details>
   ) : null}
  </div></div></div>}
