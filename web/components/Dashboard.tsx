...
function QueueActions({listing,state,onSimilar,onInterest,onDelete}:{listing:any;state:string;onSimilar:(x:any)=>void;onInterest:(x:any,restore:boolean)=>void;onDelete:(x:any)=>void}){
 ...
 const menu=open&&pos&&typeof document!=='undefined'?createPortal(<div className="queueActionMenu" style={{top:pos.top,right:pos.right}} role="menu" onClick={()=>setOpen(false)}><button onClick={()=>onSimilar(listing)}>View similar listings</button><button onClick={()=>onInterest(listing,state==='not_interested')}>{state==='not_interested'?'Resume tracking':'Not Interested in Tracking'}</button><div className="queueActionDivider"/><button className="dangerMenuItem" onClick={()=>onDelete(listing)}>Delete this listing</button></div>,document.body):null;
...
export default function Dashboard({products,listings,interventions:initialErrors,systemEvents:initialSystemEvents,opportunities:initialOpportunities,opportunityNotifications:initialOpportunityNotifications,aiEnabled}:Props){
 ...
 return <main className="appShell">
  <header className="appHeader"><div><div className="brandSmall">MOTERA RESEARCH LAB</div><div className="brandTitle cobaltWordmark">COBALT <span>V3.9.17</span></div></div><div className="headerMeta"><div className="notificationWrap"><button className={`notificationBell ${unreadOpportunityCount?'hasUnread':''}`} onClick={()=>setNotificationOpen(v=>!v)} aria-label="Opportunity signals">OPPORTUNITIES [{unreadOpportunityCount}]</button>{notificationOpen&&<div className="notificationMenu"><div className="notificationMenuHead"><strong>Opportunity signals</strong><span>{unreadOpportunityCount} unread</span></div><div className="notificationList">{opportunityNotifications.length?opportunityNotifications.map((n:any)=>{const o=opportunityById(n.opportunity_id);return <button key={n.id} className={`notificationItem ${n.read_at?'read':'unread'}`} onClick={()=>openOpportunityFromNotification(n)}><span><b>{o?.title||n.title}</b><small>{n.message}</small><em>{shortDate(n.created_at)} · {o?.signal_strength||'SIGNAL'}</em></span></button>}):<div className="notificationEmpty">No opportunity signals yet. COBALT will add corroborated product-family and high-confidence standalone signals here as evidence develops.</div>}</div></div>}</div><div className="headerClock">{clock?formatNZDateTime(clock,true):'—'}</div></div></header>
  ...
  {selectedOpportunity&&<div className="modalBack" onMouseDown={e=>{if(e.target===e.currentTarget)setSelectedOpportunity(null)}}><div className="modal opportunityModal"><div className="modalTitleBar"><div><span className="windowKicker">OPPORTUNITY SIGNAL</span><h2>{selectedOpportunity.title}</h2></div><button className="windowClose" onClick={()=>setSelectedOpportunity(null)}>×</button></div><div className="opportunityTabs"><span className="active">Overview</span><span>Supplier research</span></div><div className="modalBody opportunityBody">
   <div className={`opportunitySignalLead ${String(selectedOpportunity.signal_strength||'EMERGING').toLowerCase()} ${selectedOpportunity.opportunity_type==='standalone'?'standalone':''}`}><div><span className="opportunityEyebrow">{selectedOpportunity.opportunity_type==='standalone'?(selectedOpportunity.signal_strength==='STRONG'?'STRONG STANDALONE PRODUCT SIGNAL':'EMERGING STANDALONE PRODUCT SIGNAL'):(selectedOpportunity.signal_strength==='STRONG'?'STRONG PRODUCT SIGNAL':'EMERGING PRODUCT SIGNAL')}</span><h3>{selectedOpportunity.title}</h3><p>{selectedOpportunity.opportunity_type==='standalone'?'No reliable comparable product family found yet. This signal is based on unusually strong sustained evidence from one listing.':`${selectedOpportunity.metrics?.positive_listings||0} comparable listings are showing positive movement.`}</p></div><div className="opportunityStatus">{String(selectedOpportunity.status||'new').toUpperCase()}</div></div>
   <div className="opportunityGrid"><div className="opportunityCard"><h4>Signal evidence</h4><dl>{selectedOpportunity.opportunity_type==='standalone'?<><div><dt>Evidence class</dt><dd>Standalone listing</dd></div><div><dt>Reliable comparables</dt><dd>None yet</dd></div><div><dt>Independent evidence windows</dt><dd>{selectedOpportunity.metrics?.independent_observation_windows??'—'}</dd></div></>:<><div><dt>Comparable listings</dt><dd>{selectedOpportunity.metrics?.comparable_listings??'—'}</dd></div><div><dt>Independent positive listings</dt><dd>{selectedOpportunity.metrics?.independent_listings??'—'}</dd></div></>}<div><dt>{selectedOpportunity.opportunity_type==='standalone'?'Trusted velocity':'Median velocity'}</dt><dd>{selectedOpportunity.metrics?.median_velocity!=null?`+${selectedOpportunity.metrics.median_velocity}/day`:'—'}</dd></div><div><dt>Observed price range</dt><dd>{selectedOpportunity.metrics?.price_min!=null?`$${selectedOpportunity.metrics.price_min}${selectedOpportunity.metrics?.price_max!=null&&selectedOpportunity.metrics.price_max!==selectedOpportunity.metrics.price_min?`–$${selectedOpportunity.metrics.price_max}`:''}`:'—'}</dd></div><div><dt>Evidence window</dt><dd>{selectedOpportunity.metrics?.evidence_window_hours!=null?`${selectedOpportunity.metrics.evidence_window_hours}h`:'—'}</dd></div><div><dt>Marketplace demand score</dt><dd>{selectedOpportunity.metrics?.marketplace_demand_score??'—'}/100</dd></div><div><dt>Buyer intent score</dt><dd>{selectedOpportunity.metrics?.median_buyer_intent_score??'—'}/100</dd></div><div><dt>Purchase-intent questions</dt><dd>{selectedOpportunity.metrics?.purchase_intent_questions??0}</dd></div><div><dt>Watchers / bids</dt><dd>{selectedOpportunity.metrics?.watchers??0} / {selectedOpportunity.metrics?.bids??0}</dd></div><div><dt>Identity confidence</dt><dd>{selectedOpportunity.identity_confidence??'—'}%</dd></div></dl></div>
   ...
   <details className="classicDetails"><summary>Supporting listings ({selectedOpportunity.listings?.length||0})</summary><div className="competitorList">{(selectedOpportunity.listings||[]).map((x:any)=><div className="competitorRow" key={x.listing_uuid}><div><a href={x.listing?.url} target="_blank" rel="noreferrer">#{x.listing?.listing_id} · {x.listing?.title}</a><div className="secondary">{x.evidence?.signal||'—'} · {x.evidence?.velocity!=null?`${Number(x.evidence.velocity)>=0?'+':''}${fmt(x.evidence.velocity)}/day`:'—'} · {fmt(x.evidence?.confidence)}% confidence</div></div></div>)}</div></details>
