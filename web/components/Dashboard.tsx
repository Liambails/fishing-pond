...
export default function Dashboard({products,listings,interventions:initialErrors,systemEvents:initialSystemEvents,opportunities:initialOpportunities,opportunityNotifications:initialOpportunityNotifications,aiEnabled}:Props){
 ...
 return <main className="appShell">
  <header className="appHeader"><div><div className="brandSmall">MOTERA RESEARCH LAB</div><div className="brandTitle cobaltWordmark">COBALT <span>V3.9.15</span></div></div><div className="headerMeta"><div className="notificationWrap"><button className={`notificationBell ${unreadOpportunityCount?'hasUnread':''}`} onClick={()=>setNotificationOpen(v=>!v)} aria-label="Opportunity signals">🔔{unreadOpportunityCount>0&&<b>{unreadOpportunityCount}</b>}</button>{notificationOpen&&<div className="notificationMenu"><div className="notificationMenuHead"><strong>Opportunity signals</strong><span>{unreadOpportunityCount} unread</span></div><div className="notificationList">{opportunityNotifications.length?opportunityNotifications.map((n:any)=>{const o=opportunityById(n.opportunity_id);return <button key={n.id} className={`notificationItem ${n.read_at?'read':'unread'}`} onClick={()=>openOpportunityFromNotification(n)}><span className={`notificationDot ${o?.signal_strength==='STRONG'?'strong':''}`}></span><span><b>{o?.title||n.title}</b><small>{n.message}</small><em>{shortDate(n.created_at)} · {o?.signal_strength||'SIGNAL'}</em></span></button>}):<div className="notificationEmpty">No opportunity signals yet. COBALT will add corroborated product-family and high-confidence standalone signals here as evidence develops.</div>}</div></div>}</div><div className="headerClock">{clock?formatNZDateTime(clock,true):'—'}</div></div></header>
