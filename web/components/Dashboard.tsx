...
function HoverHistory({label,children,rows,empty='No history collected yet.'}:{label:string;children:any;rows:any[];empty?:string}){
 ...
 const show=()=>{cancelClose();cancelOpen();const anchor=anchorRef.current;if(anchor?.closest('.tableScrollLocked'))return;openTimer.current=setTimeout(()=>setOpen(true),180)};
 const hideSoon=()=>{cancelOpen();cancelClose();closeTimer.current=setTimeout(()=>setOpen(false),120)};
 useEffect(()=>{
   if(!open)return;
   const place=()=>{
...
function ListingFactsLink({listing}:{listing:any}){
 const [open,setOpen]=useState(false);const [pos,setPos]=useState<{top:number;left:number}|null>(null);const ref=useRef<HTMLAnchorElement|null>(null);const timer=useRef<ReturnType<typeof setTimeout>|null>(null);
 ...
 const close=()=>{if(timer.current)clearTimeout(timer.current);timer.current=setTimeout(()=>setOpen(false),120)};const show=()=>{if(timer.current)clearTimeout(timer.current);setOpen(true)};
...
export default function Dashboard({products,listings,interventions:initialErrors,systemEvents:initialSystemEvents,opportunities:initialOpportunities,opportunityNotifications:initialOpportunityNotifications,aiEnabled}:Props){
 ...
 const activeOpportunityCount=opportunities.filter((o:any)=>o.status!=='dismissed').length;
 ...
 const unreadOpportunityCount=opportunityNotifications.filter((n:any)=>!n.read_at).length;
 ...
 async function openOpportunityFromNotification(n:any){
  ...
  if(!n.read_at){setOpportunityNotifications(v=>v.map(x=>x.id===n.id?{...x,read_at:new Date().toISOString()}:x));await fetch('/api/opportunities',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({action:'read_notification',notificationId:n.id})});}
 }
 ...
 return <main className="appShell">
  <header className="appHeader"><div><div className="brandSmall">MOTERA RESEARCH LAB</div><div className="brandTitle cobaltWordmark">COBALT <span>V{COBALT_VERSION}</span></div></div><div className="headerMeta"><div className="notificationWrap"><button className={`notificationBell ${unreadOpportunityCount?'hasUnread':''}`} onClick={()=>setNotificationOpen(v=>!v)} aria-label={`${unreadOpportunityCount} unread sourcing opportunities`} title="Sourcing opportunities"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg>{unreadOpportunityCount>0&&<span className="notificationBadge">{unreadOpportunityCount>99?'99+':unreadOpportunityCount}</span>}</button>{notificationOpen&&<div className="notificationMenu"><div className="notificationMenuHead"><strong>Sourcing opportunities</strong><span>{unreadOpportunityCount} unread</span></div><div className="notificationList">{opportunityNotifications.length?opportunityNotifications.map((n:any)=>{const o=opportunityById(n.opportunity_id);return <button key={n.id} className={`notificationItem ${n.read_at?'read':'unread'}`} onClick={()=>openOpportunityFromNotification(n)}><span><b>{o?.title||n.title}</b><small>{n.message}</small><em>{shortDate(n.created_at)} · {o?sourcingStageLabel(o):'LEAD'}</em></span></button>}):<div className="notificationEmpty"><b>No sourcing leads yet.</b><span>COBALT will ring this bell when marketplace evidence becomes strong enough to justify checking Chinese suppliers.</span></div>}</div></div>}</div><div className="headerClock">{clock?formatNZDateTime(clock,true):'—'}</div></div></header>
  ...
  {selectedOpportunity&&<div className="modalBack" onMouseDown={e=>{if(e.target===e.currentTarget)setSelectedOpportunity(null)}}><div className="modal opportunityModal"><div className="modalTitleBar"><div><span className="windowKicker">OPPORTUNITY SIGNAL</span><h2>{selectedOpportunity.title}</h2></div><button className="windowClose" onClick={()=>setSelectedOpportunity(null)}>×</button></div><div className="opportunityTabs"><span className="active">Overview</span><span>Supplier research</span></div><div className="modalBody opportunityBody">
   <div className={`opportunitySignalLead ${sourcingStage(selectedOpportunity).toLowerCase()} ${selectedOpportunity.opportunity_type==='standalone'?'standalone':''}`}><div><span className="opportunityEyebrow">{sourcingStageLabel(selectedOpportunity)}</span><h3>{selectedOpportunity.title}</h3><p>{selectedOpportunity.opportunity_type==='standalone'?'One listing is attracting enough attention to justify supplier research, but comparable evidence is still limited.':`${selectedOpportunity.metrics?.positive_listings||0} similar listings are attracting attention at the same time.`}</p></div><div className="opportunityStatus">{String(selectedOpportunity.status||'new').toUpperCase()}</div></div>
   <div className="opportunityGrid"><div className="opportunityCard"><h4>What COBALT saw</h4><dl>{selectedOpportunity.opportunity_type==='standalone'?<><div><dt>Views since last check</dt><dd>{selectedOpportunity.metrics?.last_view_change>0?`+${selectedOpportunity.metrics.last_view_change}`:'No change recorded'}</dd></div><div><dt>Views in last 24 hours</dt><dd>{selectedOpportunity.metrics?.views_24h!=null?`+${selectedOpportunity.metrics.views_24h}`:'—'}</dd></div><div><dt>Similar listings</dt><dd>Not enough reliable matches yet</dd></div></>:<><div><dt>Similar listings gaining views</dt><dd>{selectedOpportunity.metrics?.positive_listings??'—'}</dd></div><div><dt>Total new views across them · 24h</dt><dd>{selectedOpportunity.metrics?.total_views_24h!=null?`+${selectedOpportunity.metrics.total_views_24h}`:'—'}</dd></div><div><dt>Strongest listing · 24h</dt><dd>{selectedOpportunity.metrics?.max_views_24h!=null?`+${selectedOpportunity.metrics.max_views_24h} views`:'—'}</dd></div></>}<div><dt>Observed price range</dt><dd>{selectedOpportunity.metrics?.price_min!=null?<>{`$${selectedOpportunity.metrics.price_min}${selectedOpportunity.metrics?.price_max!=null&&selectedOpportunity.metrics.price_max!==selectedOpportunity.metrics.price_min?`–$${selectedOpportunity.metrics.price_max}`:''}`}<small className="opportunityPriceNote">{selectedOpportunity.metrics?.priced_listings!=null?`${selectedOpportunity.metrics.priced_listings} of ${selectedOpportunity.metrics?.comparable_listings||1} contributing listing${Number(selectedOpportunity.metrics?.comparable_listings||1)===1?' has':'s have'} a usable price. Missing prices are excluded from pricing.`:'Missing prices are excluded from pricing.'}</small></>:<><span>Price not captured</span><small className="opportunityPriceNote">Demand evidence still counts, but COBALT will not use this listing to calculate a market price.</small></>}</dd></div>{Number(selectedOpportunity.metrics?.bids||0)>0&&<div><dt>Bids seen</dt><dd>{selectedOpportunity.metrics.bids}</dd></div>}{Number(selectedOpportunity.metrics?.purchase_intent_questions||0)>0&&<div><dt>Questions suggesting purchase intent</dt><dd>{selectedOpportunity.metrics.purchase_intent_questions}</dd></div>}{Number(selectedOpportunity.metrics?.sold_confirmations||0)>0&&<div><dt>Sold confirmations</dt><dd>{selectedOpportunity.metrics.sold_confirmations}</dd></div>}</dl></div>
