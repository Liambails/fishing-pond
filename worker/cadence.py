...
def activity_snapshot(observations):
    ...
    for prev,row in zip(independent,independent[1:]):
        ...
        gain=max(0,int((_number(row.get("views")) or 0)-(_number(prev.get("views")) or 0)))
        intervals.append({"hours":round(hours,2),"gain":gain})
    return {"observation_count":len(rows),"span_hours":round(span_h,2),"views_per_day":round(views_per_day,2) if views_per_day is not None else None,"view_delta":int(view_delta) if view_delta is not None else None,"bid_delta":int(bid_delta) if bid_delta is not None else None,"watcher_delta":int(watcher_delta) if watcher_delta is not None else None,"question_delta":int(question_delta) if question_delta is not None else None,"purchase_question_delta":int(purchase_question_delta) if purchase_question_delta is not None else None,"latest_views":last.get("views"),"latest_bids":last.get("bids"),"latest_watchers":last.get("watchers"),"view_counter_anomaly":view_counter_anomaly,"independent_count":len(independent),"independent_intervals":intervals,"recent_view_gains":[x["gain"] for x in intervals[-2:]]}

def adaptive_cadence_hours(listing, observations):
    a=activity_snapshot(observations); own=str((listing.get("metadata") or {}).get("ownership") or "").lower()=="own"; gains=a.get("recent_view_gains") or []; last=gains[-1] if gains else 0; prior=gains[-2] if len(gains)>=2 else 0; two=len(gains)>=2; bids=max(0,int(a.get("bid_delta") or 0)); watchers=max(0,int(a.get("watcher_delta") or 0)); independent=int(a.get("independent_count") or 0)
