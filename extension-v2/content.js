  ...
  function mount(){
    ...
    const capture=async()=>{
      ...
      try{
        ...
        const cfg=await settings(); const headers={'Content-Type':'application/json'}; if(cfg.token)headers['X-Cobalt-Token']=cfg.token;
        ...
        b.textContent='Saved ✓';status(`${result.continued?'Updated':'Saved'} ${result.marketplace||data.marketplace||'listing'} #${data.listing_id} · next ${result.next_observation_at?new Date(result.next_observation_at).toLocaleString():result.final_verdict||'stopped'}`);setTimeout(()=>{b.textContent='COBALT · Capture';b.disabled=false;},1000);
      }catch(e){b.textContent='COBALT · Capture';b.disabled=false;status(`Error: ${e.message}`,true);}finally{capturing=false;}
    };
